...
def create_payment():
    try:
        ...
        if paid_date_str:
            try:
                paid_date = datetime.fromisoformat(paid_date_str)
            except ValueError:
        ...
        payment = Payment(
            ...
            amount=amount,
            due_date=due_date,
            paid_date=paid_date if status == 'Completed' else None,
            status=status
        )
...
def update_payment_status(payment_id):
    ...
    try:
        ...
        # Optional: update paid_date and payment_method
        if data.get('status') == 'Completed':
            if 'paid_date' in data:
                payment.paid_date = datetime.fromisoformat(data['paid_date']) if isinstance(data['paid_date'], str) else data['paid_date']
...
def get_contract(id):
    ...
    return jsonify({
        ...
        'price': contract.price if contract.price else None,
...
def assign_apartment():
    try:
        ...
        amount = data.get('amount')
        transaction_type = data.get('transaction_type')
        payment_method = data.get('payment_method', 'Cash')  # Optional with default

        # Validation
        if not all([apartment_id, user_id, amount, transaction_type]):
            return jsonify({'error': 'Missing required fields'}), 400
        ...
        # Update apartment status based on transaction type
        ...
        return jsonify({'message': 'Apartment status updated and transaction recorded'}), 200
...
@app.route('/api/apartments/<int:apartment_id>/status', methods=['PUT'])
def set_apartment_status(apartment_id):
    data = request.get_json()
    ...
    db.session.commit()
    return jsonify({'message': f'Status set to {new_status}'})
...
def remove_apartment_buyer():
    ...
    # Delete transactions
    Transaction.query.filter_by(apartment_id=apartment_id).delete()
    ...
    db.session.commit()
    return jsonify({'message': 'Buyer/tenant removed and status reset to Available'})
